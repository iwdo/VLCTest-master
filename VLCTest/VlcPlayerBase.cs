...
namespace VlcPlayer
{
	public class VlcPlayerBase
	{
		...
		/// <param name="playArgu"></param>
		public VlcPlayerBase(string pluginPath, string playArgu)
		{
			//string pluginPath = Environment.CurrentDirectory + "\\vlc\\plugins\\";  //插件目录
			string plugin_arg = "--plugin-path=" + pluginPath;
			string[] arguments = { "-I", "dummy", "--ignore-config", "--no-video-title", plugin_arg, playArgu};
...
/*
		/// <summary>
		/// VLC 播放器带参数Command Line。
		/// </summary>
		/// <param name="pluginPath"></param>
		/// <param name="optionArgument"></param>
		public VlcPlayerArgu(string pluginPath, string optionArgument)
		{
			//string pluginPath = Environment.CurrentDirectory + "\\vlc\\plugins\\";  //插件目录
			string plugin_arg = "--plugin-path=" + pluginPath;
			string[] arguments = { "-I", "dummy", "--ignore-config", "--no-video-title", plugin_arg, optionArgument};
			libvlc_instance_ = LibVlcAPI.libvlc_new(arguments);
...
*/
		...
		public void SetOrient(string strm)
		{
			LibVlcAPI.libvlc_video_set_deinterlace(libvlc_media_player_, strm);
		}
	}
	...
	internal static class LibVlcAPI
	{
		...
		[DllImport("libvlc", CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
		[SuppressUnmanagedCodeSecurity]
		public static extern void libvlc_video_set_deinterlace(IntPtr libvlc_media_player, string mode);
	}
